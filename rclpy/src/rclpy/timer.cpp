...
Timer::Timer(
  const Clock & rclcy_clock, py::capsule pycontext, int64_t period_nsec)
{
  clock_handle_ = std::make_shared<rcl_clock_t>(*rclcy_clock.rcl_ptr());
...
define_timer(py::object module)
{
  ...
  .def(py::init<const Clock &, py::capsule, int64_t>())
