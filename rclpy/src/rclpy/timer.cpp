...
Timer::Timer(
  py::capsule pyclock, py::capsule pycontext, int64_t period_nsec)
: clock_handle_(std::make_shared<Handle>(pyclock))
{
...
define_timer(py::object module)
{
  ...
  .def(py::init<py::capsule, py::capsule, int64_t>())
