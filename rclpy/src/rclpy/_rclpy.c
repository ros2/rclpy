 ...
 * \param[in] name_space string namespace for the node
 * \return NULL on failure
...
rclpy_create_node(PyObject * Py_UNUSED(self), PyObject * args)
{
  ...
  if (!PyArg_ParseTuple(args, "ss", &node_name, &name_space)) {
    return NULL;
  }
  ...
  *node = rcl_get_zero_initialized_node();
  ...
  rcl_ret_t ret = rcl_node_init(node, node_name, name_space, &default_options);
  if (ret != RCL_RET_OK) {
    PyErr_Format(PyExc_RuntimeError,
