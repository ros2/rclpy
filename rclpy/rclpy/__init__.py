...
def spin_once(node, timeout_sec=None):
    ...
    for sub_idx in sub_ready_list:
        sub_ready = node.subscriptions[sub_idx]
        msg = _rclpy.rclpy_take(sub_ready.subscription_handle, sub_ready.msg_type)
        if msg:
            sub_ready.callback(msg)
